...
#| label = "tenleaf1",
#| echo = FALSE
m <- leaflet() %>% addProviderTiles(providers$Esri.NatGeoWorldMap)#Esri.WorldTopoMap)
m <- m %>% 
...
```{r}
#| label = "tenfinish",
#| fig.cap = "Tentsmuir study design, measuring tool top right.",
#| echo = FALSE
...
#| label = "irelandsetup",
#| echo = FALSE
m <- leaflet() %>% addProviderTiles(providers$Esri.NatGeoWorldMap)#OceanBasemap)
m <- m %>% 
  setView(-5, 54, zoom=6) %>%
  addMeasure(
    position = "bottomleft",
...
```{r}
#| label = "irelandout",
#| fig.cap = "Multiple strata for Irish survey.",
#| echo = FALSE
