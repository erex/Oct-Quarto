...
```{r}
#| echo = FALSE
scatter.smooth(size.cov.survey@dist.data$distance, size.cov.survey@dist.data$size)
```
...
detmodel.const <- make.ds.analysis(dfmodel = list(~1),
                                  ...
                                  criteria = "AIC",
                                  truncation=80)
no.size.cov <- make.simulation(reps=150, 
...
detect <- make.detectability(scale.param = 30, 
                             ...
                             truncation = 80)
plot(detect, pop.desc)
tail.size.cov <- make.simulation(reps=150, design=transects, 
                                 pop=pop.desc, det=detect,
                                 ds.analysis = detmodel.size)
...
#| echo = FALSE,
#| fig.margin = TRUE
hist(tail.size.cov.sim@results$expected.size[1,1,1:150], 
     main="Computed average group size\nGroup size covariate", 
     xlab="Group size")
...
#| echo = FALSE,
#| fig.margin = TRUE
tail.size.NOcov <- make.simulation(reps=150, design=transects, 
                                   pop=pop.desc, det=detect,
                                   ds.analysis = detmodel.const)  
...
hist(tail.size.NOcov.sim@results$expected.size[1,1,1:150], 
     main="Computed average group size\nNo covariate", 
     xlab="Group size")
abline(v=exp(ml+sl^2/2), lty=3)
```
