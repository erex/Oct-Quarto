...
#| fig.width = 7,
#| fig.asp = .75,
#| fig.cap = "Distribution of duck nest perpendicular distances. Should match the bar heights from table in Practical 1."
library(Distance)
data(ducknest)
head(ducknest, n=3)
nrow(ducknest)
...
nest.hn <- ds(ducknest, key="hn", adjustment=NULL, convert_units=conversion.factor)
summary(nest.hn)
```
...
#|   - "Hazard rate, no adjustments"
plot(nest.hn, nc=8)
plot(nest.uf.cos, nc=8)
plot(nest.hr.herm, nc=8)
```
